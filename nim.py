...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in available_actions:
            if (tuple(state), action) in self.q:
                total_q += self.q[(tuple(state), action)] 
            else:
    ...
    def choose_action(self, state, epsilon=True):
        ...
        for action in available_actions:
            if self.get_q_value(state, action) > highest_q or best_action is None or highest_q == 0:
        ...
        if epsilon == True:
            ...
            for action in available_actions:
                if action == best_action:
                    ...
                    weight = (self.epsilon / (len(available_actions) - 1))
                weights.append(weight)

            best_action = random.choices(available_actions, weights=tuple(weights))[0]
        return best_action
