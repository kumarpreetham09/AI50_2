...
def sample_pagerank(corpus, damping_factor, n):
    ...
    probabilities = transition_model(corpus, first_sample, damping_factor)

    for i in range(1 ,n):
        new_site = random.choices(list(probabilities.keys()), list(probabilities.values()), k = 1)
        dictionary[new_site[0]] += 1/n
        probabilities = transition_model(corpus, new_site[0], damping_factor)
...
def recurse_pagerank(dictionary, d, corpus):
    ...
    for page in dictionary:
        ...
        for link in corpus:
            if page in corpus[link]:
                sum_prob_i += dictionary[link] / len(corpus[link])
    ...
    # recurse
    for page in dictionary:
        if dictionary[page] - old_dictionary[page] < 0.001 and old_dictionary[page] - dictionary[page] < 0.001:
            continue
        else:
